...
```
Always: NEXT_PUBLIC_CONVEX_URL=http://mark.convex.cloud.loc
        JWT stored as: __convexAuthJWT_httpmarkconvexcloudloc
...
Your `.env.local` should use DNS names, not ports:

```bash
# Convex URLs (use DNS, not localhost:port)
NEXT_PUBLIC_CONVEX_URL=http://mark.convex.cloud.loc
NEXT_PUBLIC_CONVEX_HTTP_URL=http://mark.convex.site.loc
CONVEX_SITE_URL=http://mark.convex.site.loc

# Shared services
NOVU_API_URL=http://api.novu.loc
MAILPIT_API_URL=http://mark.mailpit.loc/api/v1
```
