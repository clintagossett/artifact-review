...
|   | - Auto-generates `NOVU_EMAIL_WEBHOOK_SECRET` | |
|   | - Sets `EMAIL_FROM_*` to staging domain | |
...
| 5.5 | Setup Novu Email Webhook (calls setup-novu-email-webhook.sh) | ~5s |
...
### 2.2.5 setup-novu-email-webhook.sh

**Purpose:** Configure Novu to send emails via Convex webhook instead of direct Resend.

**When to Run:**
- Called automatically by `agent-init.sh` (Step 5.5)
- Manually if webhook integration is missing

**Prerequisites:**
- Novu organization created (setup-novu-org.sh)
- `NOVU_EMAIL_WEBHOOK_SECRET` in `.env.convex.local` (auto-generated by agent-init.sh)

**What It Does:**

1. Check if Email Webhook integration already exists
2. Create integration via Novu API with:
   - Provider: `email-webhook` (Novu's built-in webhook provider)
   - Webhook URL: `https://{name}.convex.site.loc/novu-email-webhook`
   - HMAC secret for signature verification
3. Set integration as active and primary

**Options:**

```bash
./scripts/setup-novu-email-webhook.sh          # Create integration (idempotent)
./scripts/setup-novu-email-webhook.sh --check  # Check if configured
./scripts/setup-novu-email-webhook.sh --delete # Remove integration
```

**How Email Flow Works:**

```
Novu workflow step.email() → Novu Email Webhook provider →
POST to Convex HTTP endpoint → Convex renders React Email →
Convex sends via Resend → Mailpit (local dev)
```

**Troubleshooting:**

If emails aren't arriving:
1. Check webhook exists: `./scripts/setup-novu-email-webhook.sh --check`
2. Verify secret matches in Convex: `./scripts/setup-convex-env.sh --check | grep NOVU_EMAIL`
3. Check Convex logs: `tmux capture-pane -t {name}-convex-dev -p | grep -i novu`
...
| Novu Email Webhook | Auto-generated | .env.convex.local → Convex | agent-init.sh → setup-convex-env.sh |
| Email Addresses | Defaults to staging | .env.convex.local | agent-init.sh (editable) |
...
|--------|-----------------|-------------------|
| `agent-init.sh` | Orchestrator | Docker, Novu org, Email webhook, Convex env |
| `setup-novu-org.sh` | Orchestrator + Novu stack | Novu org + credentials |
| `setup-novu-email-webhook.sh` | Orchestrator + Novu stack | Novu Email Webhook integration |
...
### 5.9 Upgrading to New Features

**Scenario:** New infrastructure features were added (e.g., Novu Email Webhook) and you need to upgrade your existing agent.

```bash
# Re-run agent-init.sh - it's idempotent and safe
./scripts/agent-init.sh
...
- ✅ Missing secrets are auto-generated (e.g., `NOVU_EMAIL_WEBHOOK_SECRET`)
- ✅ New setup steps run if not already configured
- ✅ Everything syncs to Convex

**Total time:** ~2 minutes

**Note:** This is non-destructive. Your data, JWT keys, and sessions are preserved. The script detects what's already configured and only adds what's missing.
...
| Upgrade existing agent | `./scripts/agent-init.sh` |
