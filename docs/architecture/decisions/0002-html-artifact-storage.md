...
- ❌ Public blob storage URLs (no access control)
- ❌ Signed URLs (breaks relative paths, no granular permissions)
- ❌ Direct database serving (no Content-Type headers, complex path resolution)
...
1. **Simple HTML** → Store in Convex database (as text)
2. **Complex projects** → Store in Convex file storage (MVP) or R2 (scale)
3. **All metadata & permissions** → Store in Convex database

### Serving Strategy (Non-Negotiable)

**All files served through HTTP proxy** that:
1. Authenticates the user
2. Checks hierarchical permissions (workspace → project → document → file)
...
- ✅ **Already implemented** - Working code, no migration needed
- ✅ **Fixed costs** - Predictable pricing during growth
- ✅ **Simple architecture** - One system for DB + storage + HTTP
- ✅ **Within limits** - 100GB storage, 50GB bandwidth more than sufficient for 12+ months
- ✅ **No setup overhead** - No external accounts, API keys, or integrations
...
- Zero egress fees (Workers → R2 is internal traffic)

## Consequences

### Positive

- ✅ **Fine-grained access control** - Per-request permission checks at all levels
- ✅ **Preserves file structure** - Relative paths work naturally through proxy
- ✅ **Secure by default** - No public URLs, all files require authorization
- ✅ **Simple MVP architecture** - One system (Convex) handles everything
- ✅ **Fixed costs** - $25/month regardless of traffic during MVP phase
- ✅ **Already implemented** - No migration needed to start
- ✅ **Clear migration path** - Move to Cloudflare Workers + R2 when bandwidth exceeds limits
- ✅ **Version-friendly** - Easy to store v1/, v2/ with isolated permissions
- ✅ **Share link support** - Token-based access without user accounts

### Negative

- ⚠️ **HTTP proxy latency** - Every file request goes through permission check (unavoidable given requirements)
- ⚠️ **No CDN caching** - Files are private, can't use public CDN (acceptable tradeoff)
- ⚠️ **Future migration needed** - Must move to Workers + R2 if bandwidth grows >40GB/month
- ⚠️ **Permission query overhead** - Every file request queries database (can optimize with caching)

### Why This Decision Was Simple

**Access control requirements eliminated most alternatives:**
1. Can't use public URLs → Need authentication
2. Can't use simple signed URLs → Need hierarchical permissions (workspace/project/document/file)
3. Can't skip proxy → Need to map friendly paths to storage IDs
...
- ❌ Can't implement workspace/project/file permissions
- ❌ Can't revoke access when user leaves workspace
- ❌ No way to check share link expiration
...
**Rejected because:**
- ❌ **Breaks relative paths** - HTML expects `./app.js`, not `https://r2.../projects/123/app.js?signature=...`
- ❌ **No granular permissions** - Can't check workspace membership, project role, or document version access
- ❌ Complex to manage - Must pre-sign all referenced files
- ❌ Can't revoke access mid-session
...
**Rejected because:**
- ❌ Complex bundling logic to build and maintain
- ❌ Data URIs bloat file size significantly (base64 encoding)
- ❌ Breaks debugging/source maps
- ❌ Can't version individual files
- ❌ Still need permission checks on the single file
...
**Rejected because:**
- ❌ Must unzip on every view (slow, CPU-intensive)
- ❌ Can't check permissions on individual files
- ❌ Memory intensive for large projects
- ❌ Complex serving logic to extract and cache
...
**Rejected because:**
- ❌ Large binary files bloat database
- ❌ 1MB document size limit
- ❌ Inefficient for images/assets
- ❌ Still requires HTTP proxy for serving (no advantage)
...
- ✅ Maps friendly paths to storage IDs
...
- Whichever comes first → migrate to R2
...
- **Convex Professional plan:** $25/month (fixed)
  ...
  - Simple HTML (70 × 5KB = 350KB) → Included
  - Complex projects (30 × 20KB = 600KB) → Included
  - Bandwidth (10GB/month) → Included
- **Total: $25/month** 🎉

### Growth (1,000 projects: 700 simple, 300 complex)
- **Convex Professional plan:** $25/month (fixed)
  - Storage used: 3.5MB + 6MB = 9.5MB → Well within 100GB
  - Bandwidth (30GB/month) → Within 50GB limit
- **Total: $25/month** (no overage charges)

### Scale Phase 1 (5,000 projects, 200GB bandwidth/month)
**Option A: Stay on Convex with overages**
- Base: $25/month
- Bandwidth overage: 150GB × $0.30 = **$45/month**
- **Total: $70/month**
...
- **Total: $30.75/month** ← **Migrate here**
...
**MVP (0-12 months):**
- ✅ Current Convex implementation is **correct**
- ✅ HTTP proxy in Convex with per-request permission checks
- ✅ Files stored in Convex storage
- ✅ Fixed $25/month cost
- ✅ Quick optimizations available (delete ZIP, permission caching)

**Scale (12+ months, >40GB bandwidth/month):**
- ➡️ Migrate HTTP proxy to Cloudflare Workers
- ➡️ Move files to R2 storage
- ➡️ Keep permission logic in Convex
- ➡️ Save ~$40/month at high traffic

**Why this decision was simple:**
Access control eliminated most alternatives. The architecture is non-negotiable—only the storage backend location is flexible.
