...
export const processZipFile = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      // Get the ZIP file directly from storage (bypasses HTTP fetch networking issues)
      log.debug(LOG_TOPICS.Artifact, "Getting ZIP file from storage");
      const zipBlob = await ctx.storage.get(args.storageId);
      if (!zipBlob) {
        throw new Error("ZIP file not found in storage");
      }
      const zipBuffer = await zipBlob.arrayBuffer();
      log.debug(LOG_TOPICS.Artifact, "ZIP buffer loaded", { byteLength: zipBuffer.byteLength });
